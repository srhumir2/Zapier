...
MODIS is a sttaelite which provide mainely two kind of products. First land surface temprature (LST) and second Normilized Difference Vegetation Index (NDVI) which indicates the amount of vegetaion cover of land. This data are provided in weekley intervals. It's problem is that its is realy corce, with resolution of 960m for LST. On the other hand we had an image of the SPOT sattelite of 1.5m resolution which unfortunately does not provide LST information but fortunately provides enought data for computing NDVI. It is known that NDVI and LST have an inverse relationship. We use this fact to copmute LST for SPOT.

Briefly I computed two linear models. One to convert NDVI of SPOT to NDVI of MODIS and another to convert NDVI to LST. The first one is quite striaghtforward. But the second model had to be based on not all pixels but just the so called hot edge pixels. i.e. pixels with are among the hottest 5 percent when NDVI is binned in 0.3 unit intervals.
